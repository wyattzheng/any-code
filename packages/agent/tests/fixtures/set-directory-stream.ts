/**
 * SSE fixtures: set_working_directory tool call flow
 * OpenAI Responses API format (/v1/responses)
 *
 * Round 1: Model calls "set_working_directory" tool with {directory: <path>}
...
export function buildSetDirectoryFixtures(directory: string) {
    ...
    // ── Round 1: Model calls "set_working_directory" tool ─────────────
    const toolCallBody = [
        ...
        `data: {"type":"response.output_item.added","output_index":0,"item":{"type":"function_call","id":"fc_sd001","call_id":"call_setdir","name":"set_working_directory","arguments":"","status":"in_progress"}}\n\n`,
        `data: {"type":"response.function_call_arguments.delta","output_index":0,"item_id":"fc_sd001","delta":"${escaped}"}\n\n`,
        `data: {"type":"response.output_item.done","output_index":0,"item":{"type":"function_call","id":"fc_sd001","call_id":"call_setdir","name":"set_working_directory","arguments":"${escaped}","status":"completed"}}\n\n`,
        `data: {"type":"response.completed","response":{"id":"resp_sd001","object":"response","created_at":1700000000,"model":"gpt-4o","status":"completed","output":[{"type":"function_call","id":"fc_sd001","call_id":"call_setdir","name":"set_working_directory","arguments":"${escaped}","status":"completed"}],"usage":{"input_tokens":80,"output_tokens":30,"total_tokens":110,"output_tokens_details":{"reasoning_tokens":0},"input_tokens_details":{"cached_tokens":0}}}}\n\n`,
    ].join("")
