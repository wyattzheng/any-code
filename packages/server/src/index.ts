/**
 * any-code-server — HTTP + SSE server for CodeAgent
 *
 * Starts a lightweight HTTP server that:
 *   1. Serves a VS Code-style chat UI
...
function ensureTextBlock() {
  if (!currentTextEl) currentTextEl = el('div', 'text-block', responseContainer)
