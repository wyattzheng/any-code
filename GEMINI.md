...
- **默认框架约束**（写入 AI 系统提示词）：由于 AnyCode 的核心目的是编程，所以默认至少需要包含以下基础功能：
  - 📁 目录结构查看
  - 📄 文件内容查看
  - 📝 文件改动显示（diff view）
- AI 系统提示词中需强调生成的界面必须**移动端可用**：大字体、大按钮、适配触摸
...
- **语音输入（主要）**：点击麦克风按钮开始录音，说 "over" 关键词立即发送，无需等待静默检测
  - 点击麦克风 → 开始录音 → 说话 → 说 "over" → 立即发送给 AI
  - 也可以手动再次点击麦克风停止录音（不发送），预览识别结果后手动发送
  - 不需要一直按住按钮，点一下即可松手
- **文本输入（备选）**：在对话面板中打字发送
